# -*- coding: utf-8 -*-
"""
description:continually training chatglm1
reference:
...
class ScriptArguments:
    ...
    train_file_dir: Optional[str] = field(default='./data', metadata={"help": "The input jsonl data file folder."})
    validation_file_dir: Optional[str] = field(default='./data',
    ...
    warmup_ratio: Optional[float] = field(default=0.05, metadata={"help": "The ratio of warmup steps"})
    ...
    save_steps: Optional[int] = field(default=2000, metadata={"help": "X steps to save the model"})
    eval_steps: Optional[int] = field(default=1000, metadata={"help": "X steps to evaluate the model"})
    logging_steps: Optional[int] = field(default=1, metadata={"help": "X steps to log the model"})
    output_dir: Optional[str] = field(default="outputs-cl", metadata={"help": "The output directory"})
    max_steps: Optional[int] = field(default=5000, metadata={"help": "Number of steps to train"})
    max_epochs: Optional[int] = field(default=None, metadata={"help": "Number of epochs to train"})
...
def main():
    ...
    if args.do_train:
        ...
        train_dataset = tokenized_dataset.filter(
            lambda x: 0 < len(x['prompt'] + x['answer']) <= full_max_length
        )
        logger.debug(f"Num train_samples: {len(train_dataset)}")
        logger.debug("First train example:")
        logger.debug(train_dataset[0]['prompt'] + train_dataset[0]['answer'])
    ...
    if args.do_eval:
        ...
        logger.debug(eval_dataset[0]['prompt'] + eval_dataset[0]['answer'])
    ...
    if args.max_epochs:
        bz = args.per_device_train_batch_size * torch.cuda.device_count()
        steps_per_epoch = len(train_dataset) // bz
        max_steps = steps_per_epoch * args.max_epochs
        warmup_steps = int(max_steps * args.warmup_ratio)
        save_steps = steps_per_epoch
        eval_steps = steps_per_epoch // 2

    logger.debug(f"Training steps: {max_steps}")
    logger.debug(f"Warm up steps: {warmup_steps}")
    logger.debug(f"*"*100)
